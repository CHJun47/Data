...
def visitor_area():
        ...
        st.markdown("###### 방문자들의 거주지 분포를 시각화 한 그래프. 경기도 > 인천광역시 > 강원도 순으로 많은 유입을 보이고 있음.")
...
def visitor_seoul():
    ...
    with line1_1:
        ...
        st.markdown("###### 서울 관광객 방문자들의 Data analysis.") 
    row3_space1, row3_1, row3_space2, row3_2, row3_space3 = st.columns(
        (0.1, 1, 0.1, 1, 0.1)
    )
    with row3_1:
        st.subheader("서울 방문자 수")
        st.markdown("###### 코로나 유행기간에 급격한 하락세를 보이다 최근 들어 다시 점차 증가하는 추세를 보임.")
...
def inflow_ouflow_chart():
    ...
    with row4_1:
        ...
        st.markdown("###### 서울의 유입 유출 방문객의 데이터 분석을 통해 경기도 유입은 86.9% ,유출은 85.5%, 인천광역시 유입9.9% ,유출8.2%, 강원도 유입1.1% ,유출3.7%를 나타내고 있음.")
...
def touristDest():
    ...
    with row5_1:
        ...
        st.markdown("###### 서울의 관광객 방문자들의 소비 유형 파악.") 
